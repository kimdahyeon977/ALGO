...
def recommend(flag,heap):
    flag = -flag
    while heap and (heap[0][1]*flag not in problem_dict.keys() or problem_dict[heap[0][1]*flag] != heap[0][0]*flag): #(1) 없어진 문제 번호가 다 없어질때까지 or #(2) 이미 제거된 문제이지만, 난이도가 다른 경우는 지우지 x
        heapq.heappop(heap)     
    result = heap[0]
